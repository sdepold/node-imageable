...
RequestHandler.prototype._getTempFolderSize = function(callback) {
  console.log('ls -ilah ' + this.config.tmpPathRoot)
  this.utils.exec('ls -ilah ' + this.config.tmpPathRoot, function(err, stdout, stderr) {
    console.log(stdout)
    this.utils.exec('du -csm ' + this.config.tmpPathRoot, function(err, stdout, stderr) {
      console.log(stdout)
      callback && callback(stdout.match(/(\d+)/)[1])
    }.bind(this))
  }.bind(this))
}
