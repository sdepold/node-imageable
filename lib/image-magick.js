...
ImageMagick.Templates = {
  ...
  downloadCmd: "curl -A " + userAgent + " -e %{sourceHost} --create-dirs -sf '%{source}' -o '%{target}'",
...
ImageMagick.prototype._downloadSource = function(source, target, callback) {
  ...
  var sourceHost = url.parse( source ).host;
  this.execute(ImageMagick.Templates.get('downloadCmd', {source: source, sourceHost: sourceHost, target: target}), { timeout: this.config.timeouts.download }, callback)
}
