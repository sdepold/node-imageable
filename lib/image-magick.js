...
var ImageMagick = module.exports = {
  ...
  _sourceExists: function(source, callback) {
    if(source.indexOf("http") == 0) {
      exec("curl --head -s '"+ source +"'|grep HTTP", function(err, stdout, stderr) {
        callback && callback(stdout.indexOf('200 OK') > -1)
      })
    } else {
      path.exists(source, callback)
    }
  },
    ...
    ImageMagick._sourceExists(source, function(exists) {
      ...
      ImageMagick._getTempfile(source, function(tmpfile) {
        ...
        var command = exists 
          ...
          : (function() {
              size = size.replace(">", "")
              size = size.split('x').map(function(num){ return (num == "") ? "" : (parseInt(num) - 2) }).join("x")
              return ["convert -bordercolor '#B1B1B1' -background '#eee' -border 1 -size", size, "-fill '#900' -font '/Library/Fonts/Arial Bold.ttf' -pointsize 16 label:' x' " + tmpfile].join(" ")
            })()
