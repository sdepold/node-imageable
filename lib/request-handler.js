...
RequestHandler.prototype._getTempFolderSize = function(callback) {
  ...
  var returnSize = function() {
    if((fileCount !== null) && (dirSize !== null) && callback) {
      console.log('fileCount', fileCount, 'dirSize', dirSize)
      callback((fileCount === 0) ? 0 : dirSize)
    }
  }.bind(this)

  this.utils.exec('ls -ilA ' + this.config.tmpPathRoot + '|wc -l', function(err, stdout, stderr) {
    fileCount = stdout.match(/(\d+)/)[1]
    returnSize()
  }.bind(this))

  this.utils.exec('du -csm ' + this.config.tmpPathRoot, function(err, stdout, stderr) {
    dirSize = stdout.match(/(\d+)/)[1]
    returnSize()
  }.bind(this))
}
