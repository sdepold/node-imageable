var exec = require('child_process').exec
  , path = require('path')
  , IdentifyParser = require('./identify-parser')
...
ImageMagick.prototype.identify = function(params, callback) {
  ...
  var command = ImageMagick.Templates.get('identifyVerboseCmd', {file: params.url})

  this.execute(command, { timeout: this.config.timeouts.identify }, function(err, stdout, stderr) {
    var result = stdout

    if(!err)
      result = IdentifyParser.parse(result)
