...
ImageMagick.prototype._scaleCropParam = function(params, callback) {
  ...
  this.identify({url: params.url}, function(err, stats) {
    if(err)
      ...
      params.crop = self._scaleCropInfo(params.crop, params.cropSourceSize, stats.Geometry.split('+')[0].split('x'))
      callback && callback()
    }
  })
}

ImageMagick.prototype._scaleCropInfo = function(cropValue, _cropSourceSize, sourceDimension) {
  var cropSourceSize   = _cropSourceSize.split('+')[0].split('x')
    , cropSourceWidth  = cropSourceSize[0]
